...
Global keyboard shortcuts can be remapped in a separate `keybindings.json` file in the same
directory. See [Customizing Global Keybindings](./keybindings#customizing-global-keybindings).
...
| aipanel:provider                     | string   | which provider backs the left AI panel, also selectable from the panel's provider dropdown: "wave" (default, the built-in Wave AI chat), "claude" (claude.ai), "codex" (ChatGPT Codex), or "gemini" (Google Gemini), each embedded via a webview |
...
| term:dynamictitle <VersionBadge version="v0.15" />                  | bool     | when enabled, the terminal block header shows the running foreground process (e.g. `vim`, `Claude Code`) and reverts to the default when idle (defaults to true). A `frame:text` override always takes precedence                                              |
