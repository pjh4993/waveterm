...
Wave detects that Claude Code is running, but it has no way to know your session's name on its own — so a hook supplies it. The hook derives a short title and writes it to the block's `frame:text`, which overrides the automatic label. On session end it clears `frame:text`, so the title reverts to the default.

### 1. Add the helper script

Save this as `~/.config/waveterm/wave-claude-title.sh` and make it executable (`chmod +x`). It needs [`jq`](https://jqlang.github.io/jq/).

```bash
#!/usr/bin/env bash
# Set the Wave terminal panel title to the Claude Code session name.
# Reads the Claude Code hook JSON on stdin.
set -euo pipefail

payload=$(cat)

# Prefer an explicit session title (set after /resume or `claude -n <name>`).
title=$(printf '%s' "$payload" | jq -r '.session_title // empty')

# Otherwise fall back to the first user message in the transcript.
if [ -z "$title" ]; then
    transcript=$(printf '%s' "$payload" | jq -r '.transcript_path // empty')
    if [ -n "$transcript" ] && [ -f "$transcript" ]; then
        title=$(jq -r 'select(.type=="user") | (.message.content // .content)
            | if type=="array" then (.[0].text // "") else . end' "$transcript" 2>/dev/null \
            | grep -v '^[[:space:]]*$' | head -n1 | cut -c1-60)
    fi
fi
...
wsh setmeta -b this "frame:text=✳ ${title}"
```

### 2. Wire up the hooks

Add these to `~/.claude/settings.json` (merge into your existing `hooks` block — they sit alongside the badge hooks above):
    ...
    "SessionStart": [
      {
        "matcher": "*",
        "hooks": [
          { "type": "command", "command": "~/.config/waveterm/wave-claude-title.sh" }
        ]
      }
    ],
    "UserPromptSubmit": [
    ...
    "SessionEnd": [
      {
        "matcher": "*",
        "hooks": [
          { "type": "command", "command": "wsh setmeta -b this frame:text=" }
        ]
      }
    ]
  }
...
`SessionStart` sets the title when the session begins or resumes, `UserPromptSubmit` keeps it fresh as the conversation evolves, and `SessionEnd` clears it (`frame:text=` with an empty value deletes the key, reverting to the default title).

:::info
`wsh setmeta -b this` targets the terminal block that the hook is running inside. The title is stored on the block (`frame:text`), so it survives tab switches and works across splits. If you ever want to pin a manual title that the automatic label won't override, set it yourself: `wsh setmeta -b this frame:text="My Title"`.
:::
