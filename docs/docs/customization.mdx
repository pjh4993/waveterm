...
By default, the terminal block header shows the foreground process currently running (e.g. `vim`, `npm`, `Claude Code`) and reverts to the default when the shell is idle. Set the config key `term:dynamictitle` to `false` to disable this. For Claude Code you can show the actual session name instead of the generic label — see [Claude Code Integration](/claude-code#showing-the-session-name-as-the-panel-title).
